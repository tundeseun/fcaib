...
class LessonPlanController extends Controller
{
    ...
    public function lessonPlannerSearch(Request $request){
        {
            ...
            try {
                $data = $this->loadDefault();	
                $user = Auth::user();
                $data['class_times'] = SmClassTime::where('type','class')->where('school_id', Auth::user()->school_id)->orderBy('period', 'ASC')->get();
    ...
    public function changeWeek(Request $request,$teacher_id,$next_date){
			...
			$user = Auth::user();
			$class_times = SmClassTime::where('type','class')->where('school_id', Auth::user()->school_id)->orderBy('period', 'ASC')->get();
            if($user->role_id == 4){
    ...
    public function teacherLessonPlanOverview(){
        if(Auth::user()->role_id==4){
            try {
                ...
                $user = Auth::user();
                $class_times = SmClassTime::where('type','class')->where('school_id', Auth::user()->school_id)->orderBy('id', 'ASC')->get();
