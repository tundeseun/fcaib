...
class SmTopicController extends Controller
{
    ...
    //  return $request->all();
    	$request->validate(
    	   ...
    	   DB::beginTransaction();
           $is_duplicate = SmLessonTopic::where('school_id', Auth::user()->school_id)->where('class_id', $request->class)->where('lesson_id',$request->lesson)->where('section_id', $request->section)->where('subject_id', $request->subject)->first();
          if($is_duplicate){
    ...
    public function edit($id){
        ...
        $data=$this->loadTopic();
         ...
         $data['topicDetails']=SmLessonTopicDetail::where('topic_id',$data['topic']->id)->where('school_id', Auth::user()->school_id)->get();
        $data['id']=$id;
    ...
    public function loadTopic(){
        ...
        if(Auth::user()->role_id==4){
             ...
             $data['topics'] = SmLessonTopic::with('lesson','class','section','subject')->whereIn('subject_id',$subjects)->where('school_id', Auth::user()->school_id)->get();

        }else{
             $data['topics'] = SmLessonTopic::with('lesson','class','section','subject')->where('school_id', Auth::user()->school_id)->get();
        }
        ...
        if (teacherAccess()) {
            ...
            $data['classes'] = SmClass::where('active_status', 1)->where('school_id',Auth::user()->school_id)->get();
         
        }
        $data['subjects'] = SmSubject::where('active_status', 1)->where('school_id', Auth::user()->school_id)->get();
        $data['sections'] = SmSection::where('active_status', 1)->where('school_id', Auth::user()->school_id)->get();
        return $data;
    }
}
