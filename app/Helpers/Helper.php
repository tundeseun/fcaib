...
if (!function_exists('getAcademicId')){
    function getAcademicId(){
        ...
        if (session()->has('sessionId')) {
            ...
            $session = SmGeneralSettings::where('id', 1)->first()->academic_id;
...
if (!function_exists('markGpa')) {
    ...
    {
        $mark = SmMarksGrade::where([['percent_from', '<=', floor($marks)], ['percent_upto', '>=', floor($marks)]])->where('school_id',Auth::user()->school_id)->first();
        if ($mark)
            return $mark;
        else
            return '';
...
if (!function_exists('getGrade')) {
    ...
    {
        $mark = SmMarksGrade::where('from', '<=', $grade)->where('up', '>=', $grade)->first();
        if ($mark)
            return $mark;
        else
            return '';
...
if (!function_exists('getExamResult')) {
    ...
    {
        $eligible_subjects = SmAssignSubject::where('class_id', $student->class_id)->where('section_id', $student->section_id)
    ...
    function getValueByStringTestRegistration($data, $str)
    {
        ...
        if ($data['slug'] == 'student') {
            ...
            if ($str == 'name') {
                ...
                return @$student_info->guardian_name ?? "N/A";
            } elseif ($str == 'class') {
                return @$student_info->class->class_name ?? "N/A";
            } elseif ($str == 'section') {
                return @$student_info->section->section_name ?? "N/A";
            }
        } elseif ($data['slug'] == 'parent') {
            $parent_info = SmStudentRegistration::find(@$data['id']);
            if ($str == 'name') {
                return @$parent_info->guardian_name ?? "N/A";
...
if (!function_exists('getValueByStringTestReset')) {
    function getValueByStringTestReset($data, $str)
    {
        if ($str == 'school_name') {
            ...
            $general_setting = SmGeneralSettings::find(1);
            return @$general_setting->school_name ?? "N/A";
        } elseif ($str == 'name') {
...
if (!function_exists('subjectFullMark')){
    function subjectFullMark($examtype, $subject){
        try{
            $full_mark = 0;
            $full_mark = SmExam::where('school_id',Auth::user()->school_id)->where('exam_type_id', $examtype)->where('subject_id',$subject)->first('exam_mark')->exam_mark;
            return $full_mark;
        }
        catch (\Exception $e) {
            return '' ;
...
if (!function_exists('getSessionById')) {
    ...
    {
        return DB::table('sm_academic_years')->where('id', $id)->first()->title ?? "N/A";
    }
...
if (!function_exists('getDepartmentById')) {
    ...
    {
        return DB::table('sm_classes')->where('id', $id)->first()->class_name ?? "N/A";
    }

}

if (!function_exists('getLevelById')) {
    function getLevelById($id)
    {
        return DB::table('sm_sections')->where('id', $id)->first()->section_name ?? "N/A";
    }

}

if (!function_exists('getStudentsByDept')) {
    function getStudentsByDept($id)
    {
        return DB::table('sm_students')->where('class_id', $id)->count();
    }

}

function getCost($paymentType){
    return DB::table('sm_fees_masters')->where('sm_fees_masters.id',$paymentType)
