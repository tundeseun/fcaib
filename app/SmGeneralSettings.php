...
class SmGeneralSettings extends Model
{
    ...
    public static function make_merit_list($InputClassId, $InputSectionId, $InputExamId)
    {
        try {
            ...
            $allresult_data = SmTemporaryMeritlist::where('iid', $iid)->orderBy('gpa_point', 'desc')->get();
            $merit_serial = 1;
            ...
            $allresult_data = SmTemporaryMeritlist::where('iid', $iid)->orderBy('merit_order', 'asc')->get();
            $data['iid'] = $iid;
            $data['exams'] = $exams;
