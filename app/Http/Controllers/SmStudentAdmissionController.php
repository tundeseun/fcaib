...
use App\Imports\GraduandsImport;
use App\SmGraduandsList;
...
class SmStudentAdmissionController extends Controller
{
    ...
    public function studentStore(request $request)
    {
        ...
        if (SmStudent::where('email', $request->student_email)->exists() || User::where('email', $request->student_email)->exists()) {
            $validator->getMessageBag()->add('student_email', 'Email already registered');
        }

        if (SmStudent::where('utme_number', $request->utme_number)->exists()) {
        ...
        try {
            ...
            $ac_year = SmAcademicYear::where('id', $request->academic_year)->first()->year;

            $created_year = $ac_year . '-01-01 12:00:00';

            $matric_number = 'NDAG/' . $ac_year . '/' . str_pad($request->admission_number, 4, '0', STR_PAD_LEFT);
            ...
            if ($request->file('document_file_1') != "") {
                ...
                $file = $request->file('document_file_1');
                $student->document_file_1 = 'UTME-' . md5($file->getClientOriginalName() . time()) . "." . $file->getClientOriginalExtension();
                $file->move('public/uploads/student/document/', $student->document_file_1);
                $student->document_file_1 =  'public/uploads/student/document/' . $student->document_file_1;
            }

            $student->document_title_2 = 'SSCE Result';
            $student->document_file_2 = "";
            if ($request->file('document_file_2') != "") {
                ...
                $file = $request->file('document_file_2');
                $student->document_file_2 = 'SSCE-' . md5($file->getClientOriginalName() . time()) . "." . $file->getClientOriginalExtension();
                $file->move('public/uploads/student/document/', $student->document_file_2);
                $student->document_file_2 =  'public/uploads/student/document/' . $student->document_file_2;
            }

            $student->document_title_3 = 'Guarantors Letter';
            $student->document_file_3 = "";
            if ($request->file('document_file_3') != "") {
                ...
                $file = $request->file('document_file_3');
                $student->document_file_3 = 'GF-' . md5($file->getClientOriginalName() . time()) . "." . $file->getClientOriginalExtension();
                $file->move('public/uploads/student/document/', $student->document_file_3);
                $student->document_file_3 =  'public/uploads/student/document/' . $student->document_file_3;
            }
    ...
    public function studentView(Request $request, $id)
    {
        ...
        try {
            ...
            $fees_assigneds = SmFeesAssign::where('student_id', $id)->where('school_id', Auth::user()->school_id)->get();
            //  return $fees_assigneds;
            $fees_discounts = SmFeesAssignDiscount::where('student_id', $id)->where('school_id', Auth::user()->school_id)->get();
            // $documents = SmStudentDocument::where('student_staff_id', $id)->where('type', 'stu')->where('school_id',Auth::user()->school_id)->get();
            $documents = SmStudentDocument::where('student_staff_id', $id)->where('school_id', Auth::user()->school_id)->get();
            ...
            return view('backEnd.studentInformation.student_view', compact('student_detail', 'fees_assigneds', 'fees_discounts', 'documents', 'academic_year', 'accomodation'));
        } catch (\Exception $e) {
            Toastr::error('Operation Failed', 'Failed');
            return redirect()->back();
        }
    ...
    public function studentUpdate(Request $request)
    {
        ...
        if ($validator->fails()) {
            ...
            if ($request->password != "") {
                $user_stu->password = Hash::make($request->password);
            }
            ...
            if ($student->section_id !== $request->section) {
                $student->first_semester_reg = 0;
                $student->second_semester_reg = 0;
                $student->first_course_registration    = NULL;
                $student->second_course_registration = NULL;
                $student->section_id = $request->section;
            } else {
                $student->section_id = $request->section;
            }
    ...
    {
        $students = SmStudent::where('class_id', $request->class)->where('section_id', $request->section)->where('active_status', 1)->where('school_id', Auth::user()->school_id)->get(['id', 'full_name', 'user_id']);
        ...
        try {
            ...
            if ($passStudents != 0) {
                foreach ($passStudents['allresult_data'] as $key => $value) {
                    $d = SmStudent::where('id', $value->student_id)->first();
                    if ($d->count() != 0) {
    ...
    public function studentCurrentSearchCustom(Request $request)
    {
        ...
        try {
            ...
            $students = SmStudent::where('class_id', '=', $request->current_class)->where('session_id', '=', $request->current_session)->where('section_id', $request->section)->where('active_status', 1)->where('school_id', Auth::user()->school_id)->get();
    ...
    public function studentPromoteStore(Request $request)
    {
        ...
        try {
            ...
            if ($request->promote_class == "" || $request->promote_session == "") {
                $students = SmStudent::where('class_id', '=', $request->promote_class)->where('session_id', '=', $request->promote_session)->where('school_id', Auth::user()->school_id)->get();
                ...
                try {
                    ...
                    foreach ($request->student_checked as $student_id) {
                        ...
                        if ($request->result[$student_id] == 'P') {
                            $merit_list = \App\SmTemporaryMeritlist::where(['student_id' => $student_id, 'class_id' => $request->current_class, 'section_id' => $student_details->section_id])->first();
                    ...
                    DB::rollback();
                    $students = SmStudent::where('class_id', '=', $request->current_class)->where('session_id', '=', $request->current_session)->where('school_id', Auth::user()->school_id)->get();
    ...
    public function studentAttendanceReportSearch(Request $request)
    {
        ...
        try {
            ...
            $students = SmStudent::where('class_id', $request->class)->where('section_id', $request->section)->where('active_status', 1)->where('school_id', Auth::user()->school_id)->get();
        ...
        if ($file_type <> 'csv' && $file_type <> 'xlsx' && $file_type <> 'xls') {
            ...
            if (!empty($data)) {
                foreach ($data as $key => $value) {
                    ...
                    // if(SmStudent::where('matric_number', $value->matric_number)->exists() && $value->matric_number != null) {
                    //     $error = 0;
                    // }
                    ...
                    if ($error == 1) {
                        ...
                        $user_stu->username = $value->matric_number;
                        ...
                        $student->user_id = $user_stu->id;
                        $student->role_id = 2;
                        $student->admission_no = substr($value->matric_number, -4);
                        $student->matric_number = $value->matric_number;
                        $student->first_name = $value->first_name;
                        $student->last_name = $value->last_name;
                        $student->full_name = $value->first_name . ' ' . $value->last_name;
                        $student->gender_id = strtolower($value->gender) == 'male' ? 1 : 2;
                        ...
                        $student->guardian_name = $value->guardian_name;
                        $student->guardian_email = $value->guardian_email;
                        $student->guardian_mobile = $value->guardian_mobile;
                        $student->guardian_relation = strtoupper(substr($value->guardian_relation, 0, 1)) ?? 'O';
                ...
                DB::commit();
                Toastr::success("{$failCount} failed & {$successCount} successful", 'Success');
                return redirect()->back();
            } else {
                Toastr::warning('The CSV file must contain valid data', 'Warning');
                return redirect()->back();
            }
        }
    }
    ...
    {
        try {
            $students = SmStudent::where('active_status', 1)->where('school_id', Auth::user()->school_id)->get();
            $classes = SmClass::where('active_status', 1)->where('school_id', Auth::user()->school_id)->get();
    ...
    {
        try {
            $transcripts = DB::table('sm_transcript_applications')->where('status', 0)->get();
            return view('backEnd.studentPanel.transcript_applications', compact('transcripts'));
        } catch (\Exception $e) {
            Toastr::error('Operation Failed', 'Failed');
            return redirect()->back();
        }
    ...
    public function transcriptUapplications()
    {
        try {
            $transcripts = DB::table('sm_transcript_applications')->where('status', 1)->get();
            return view('backEnd.studentPanel.transcript_uapplications', compact('transcripts'));
        } catch (\Exception $e) {
            Toastr::error('Operation Failed', 'Failed');
            return redirect()->back();
        }
    ...
    public function transcriptSingle($id)
    {
        try {
            $transcript = DB::table('sm_transcript_applications')->where('id', $id)->first();
            return view('backEnd.studentPanel.transcript_single', compact('transcript'));
        } catch (\Exception $e) {
            Toastr::error('Operation Failed', 'Failed');
            return redirect()->back();
        }
    ...
    public function transcriptTreated($id)
    {
        try {
            DB::table('sm_transcript_applications')->where('id', $id)->update(['status' => 1]);
            Toastr::success('Operation Successful', 'Success');
            return redirect()->back();
        } catch (\Exception $e) {
            Toastr::error('Operation Failed', 'Failed');
    ...
    public function transcriptUntreated($id)
    {
        try {
            DB::table('sm_transcript_applications')->where('id', $id)->update(['status' => 0]);
            Toastr::success('Operation Successful', 'Success');
            return redirect()->back();
        } catch (\Exception $e) {
            Toastr::error('Operation Failed', 'Failed');
    ...
    {
        $marks = DB::table('sm_marks_grades')->orderBy('grade_name')->get();
        $details = DB::table('sm_transcript_applications')->where('id', $id)->first();
        $levels = SmCourseRegistration::where('student_id', '=', $details->student_id)
            ->where('uploaded', '=', 1)
        ...
        $pdf = \App::make('dompdf.wrapper');
        $pdf->loadHTML(view("parentregistration::generate_transcript", compact('details', 'levels', 'marks'))->render());
        return $pdf->stream();
    }

    public function statementUntreated()
    {
        try {
            $results = DB::table('sm_statement_result_applications')->where('status', 0)->get();
            return view('backEnd.studentPanel.statement_uapplications', compact('results'));
        } catch (\Exception $e) {
            Toastr::error('Operation Failed', 'Failed');
            return redirect()->back();
        }
    ...
    public function statementTreated()
    {
        try {
            $results = DB::table('sm_statement_result_applications')->where('status', 1)->get();
            return view('backEnd.studentPanel.statement_applications', compact('results'));
        } catch (\Exception $e) {
            Toastr::error('Operation Failed', 'Failed');
            return redirect()->back();
        }
    ...
    public function statementMTreated($id)
    {
        try {
            DB::table('sm_statement_result_applications')->where('id', $id)->update(['status' => 1]);
            Toastr::success('Operation Successful', 'Success');
            return redirect()->back();
        } catch (\Exception $e) {
            Toastr::error('Operation Failed', 'Failed');
    ...
    public function statementMUntreated($id)
    {
        try {
            DB::table('sm_statement_result_applications')->where('id', $id)->update(['status' => 0]);
            Toastr::success('Operation Successful', 'Success');
            return redirect()->back();
        } catch (\Exception $e) {
            Toastr::error('Operation Failed', 'Failed');
    ...
    {
        try {
            $courses = SmSubject::where('active_status', 1)->where('school_id', Auth::user()->school_id)->get();
            return view('backEnd.studentPanel.upload_results', compact('courses'));
        } catch (\Exception $e) {
            Toastr::error('Operation Failed', 'Failed');
            return redirect()->back();
        }
    ...
    public function uploadResultStore(Request $request)
    {
        ...
        if ($file_type <> 'csv' && $file_type <> 'xlsx' && $file_type <> 'xls') {
            ...
            DB::beginTransaction();
            $path = $request->file('file');
            Excel::import(new ResultsImport, $request->file('file'), 's3', \Maatwebsite\Excel\Excel::XLSX);
            $data = ResultBulkTemporary::where('user_id', Auth::user()->id)->get();

            if (!empty($data)) {
                $course = SmSubject::find($request->course);
                foreach ($data as $key => $value) {
                    //dd($value->matric_number);
                    if (!SmStudent::where('matric_number', $value->matric_number)->exists()) {
                        $error = 0;
                    }

                    if ($error == 1) {
                        $grade                      = SmMarksGrade::where([['percent_from', '<=', $value->score], ['percent_upto', '>=', $value->score]])->first();
                        $grade_point                = $course->units * $grade->gpa;
                        //
                        SmCourseRegistration::updateOrCreate([
                            'subject_id'   => $course->id,
                            'student_id'   => SmStudent::where('matric_number', $value->matric_number)->first()->id,
                            'semester'     => $course->subject_type,
                ...
                ResultBulkTemporary::where('user_id', Auth::user()->id)->delete();
                DB::commit();
                Toastr::success("{$failCount} failed & {$successCount} successful", 'Success');
                return redirect()->back();
            } else {
    ...
    public function uploadGraduands()
    {
        try {
            $departments = SmClass::where('active_status', 1)->where('school_id', Auth::user()->school_id)->get();
            return view('backEnd.studentPanel.upload_graduands', compact('departments'));
        } catch (\Exception $e) {
    ...
    public function uploadGraduandsStore(Request $request)
    {
        $request->validate(
            [
                'session' => 'required',
        ...
        if ($file_type <> 'csv' && $file_type <> 'xlsx' && $file_type <> 'xls') {
            ...
            DB::beginTransaction();
            $path = $request->file('file');
            Excel::import(new GraduandsImport, $request->file('file'), 's3', \Maatwebsite\Excel\Excel::XLSX);
            $data = GraduandBulkTemporary::where('user_id', Auth::user()->id)->get();
            //dd($data);
            if (!empty($data)) {
                $class_id = $request->department;
                foreach ($data as $key => $value) {
                    //dd($value->matric_number);
                    if (!SmStudent::where('matric_number', $value->matric_number)->exists()) {
                        $error = 0;
                    ...
                    if ($error == 1) {
                        SmGraduandsList::updateOrCreate([
                            'class_id'      => $class_id,
                            'student_id'    => SmStudent::where('matric_number', $value->matric_number)->first()->id,
                            ...
                            'cgpa'              => $value->cgpa,
                            'class_of_degree'   => $value->class_of_degree,
                            'matric_number'     => $value->matric_number,
    ...
    {
        try{
                $students = SmStudent::orderBy('class_id', 'asc')
                                        ->get();

                $academic_year = new SmAcademicYear();
                $academic_year->year = date('Y');
                $academic_year->title = date('Y').'/'.date('Y')+1;
                $academic_year->starting_date = now()->format('Y-m-d');
                $academic_year->ending_date = now()->addMonths(11)->format('Y-m-d');
                ...
                SmCourseRegistration::where('uploaded', 0)->update(['uploaded' => 1]);

                foreach($students as $student){
                    $class = $student->class_id;
                    $section = $student->section_id;
                    $nextSection = SmClassSection::where('class_id', $class)
                    ...
                    if(!$nextSection){
                            SmGraduandsList::updateOrCreate([
                                ...
                                'class_id'          => $student->class_id,
                                'student_id'        => $student->id,
                                'session'           => $academic_year->title,
                                'cgpa'              => SmCourseRegistration::cgpa($student->id),
                                'class_of_degree'   => trim(substr($student->section->section_name, 0, 3)) == 'ND' ? 'National Diploma' : 'Higher National Diploma',
                                'matric_number'     => $student->matric_number,
                                'academic_id'       => getAcademicId()
                            ]);
                    }else{
                        ...
                        $student->first_course_registration = NULL;
                        $student->second_course_registration = NULL;
                        $student->section_id = $nextSection->section_id;
                        $student->room_id = 0;
                        $student->session_id = $academic_year->id;
                        $student->save();
                    }
            
            }

            Toastr::success("Mass promotion successful", 'Success');
            return redirect()->back();
        
        }catch(Exception $e)
        {
            Toastr::error("Mass promotion failed, ".$e->getMessage(), 'Success');
            return redirect()->back();
        }
