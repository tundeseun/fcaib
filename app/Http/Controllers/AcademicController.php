    ...
    public function classReportSearch(Request $request)
    {
        ...
        try {
            ...
            foreach ($students as $student) {
                $fees_assigns = SmFeesAssign::where("student_id", $student->id)->where('active_status', 1)->where('school_id', Auth::user()->school_id)->get();
                foreach ($fees_assigns as $fees_assign) {
                    $fees_masters = SmFeesMaster::where('id', $fees_assign->fees_master_id)->where('school_id', Auth::user()->school_id)->get();
