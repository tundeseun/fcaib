...
class SmPayrollController extends Controller
{
	...
	public function generatePayroll(Request $request, $id, $payroll_month, $payroll_year)
	{
		...
		try{
			...
			$staff_leaves = SmLeaveDefine::where('user_id',$staffDetails->user_id)->where('role_id', $staffDetails->role_id)->where('school_id',Auth::user()->school_id)->get();
			$staff_leave_deduct_days = SmLeaveDeductionInfo::where('staff_id', $id)->where('pay_year', $payroll_year)->where('school_id',Auth::user()->school_id)->get()->sum("extra_leave");
