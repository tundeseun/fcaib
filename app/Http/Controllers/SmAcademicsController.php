...
class SmAcademicsController extends Controller
{
    ...
    public function manageDocuments(){
        try {
            $documents = SmStudentDocument::where('student_staff_id', 0)->get(); 
            return view('backEnd.academics.documents', compact('documents'));
        } catch (\Exception $e) {
            Toastr::error('Operation Failed', 'Failed');
            return redirect()->back();
        }
        ...
        public function addDocument(Request $request)
    {
       ...
       if(empty($request->title) || empty($request->file('photo')) || empty($request->purpose)){
