...
class SmOnlineExamController extends Controller
{
    ...
    public function studentOnlineExam()
    {
        try {
            ...
            $online_exams = SmOnlineExam::where('status', 1)->where('class_id', $student->class_id)
                                        ->join('sm_course_registrations', function($join){
                                            $join->on('sm_course_registrations.subject_id','=', 'sm_online_exams.subject_id')
                                                ->where('sm_course_registrations.uploaded','=',0);
                                        })
                                        ->select('sm_online_exams.*','sm_course_registrations.subject_id','sm_course_registrations.uploaded')
                                        ->get();

            //dd($online_exams);
        
             $marks_assigned =  SmStudentTakeOnlineExam::whereIn('online_exam_id', $online_exams->pluck('id')->toArray())
