...
class SmFeesTypeController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        try{
            $remitaFees = Remita::getStaticBillerServices();
            ...
            return view('backEnd.feesCollection.fees_type', compact('fees_types','fees_groups', 'remitaFees'));
        }catch (\Exception $e) {
    ...
    public function store(Request $request)
    {
        ...
        $feeName = Remita::getStaticBillerServices($request->name);
        $is_duplicate = SmFeesType::where('name', $feeName)->where('fees_group_id', $request->fees_group)->first();
        if ($is_duplicate) {
        ...
        try{
            ...
            $fees_type->name = $feeName;
            $fees_type->remita_service_id = $request->name;
    ...
    public function edit(Request $request, $id)
    {
        try{
            ...
            return view('backEnd.feesCollection.fees_type', compact('fees_type', 'fees_types','fees_groups', 'remitaFees'));
        }catch (\Exception $e) {
           Toastr::error('Operation Failed', 'Failed');
           return redirect()->back();
        }
    ...
    public function update(Request $request)
    {
        ...
        $feeName = Remita::getStaticBillerServices($request->name);
        $is_duplicate = SmFeesType::where('id','!=', $request->id)->where('name', $feeName)->where('fees_group_id', $request->fees_group)->first();
        if ($is_duplicate) {
            Toastr::error('Duplicate name found!', 'Failed');
            return redirect()->back()->withErrors($validator)->withInput();
        }
        ...
        try{
            $fees_type = SmFeesType::where('id',$request->id)->first();
            $fees_type->name = $feeName;
            $fees_type->remita_service_id = $request->name;
            $fees_type->fees_group_id = $request->fees_group;
            $fees_type->description = $request->description;
