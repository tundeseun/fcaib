...
class SmAssignClassTeacherControler extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        try {
            $assigned_class_teacher = SmAssignClassTeacher::where('active_status', 1)->where('class_id', $request->class)->where('section_id', $request->section)->where('school_id', Auth::user()->school_id)->first();
    ...
    public function update(Request $request, $id)
    {
        ...
        $is_duplicate = SmAssignClassTeacher::where('school_id', Auth::user()->school_id)->where('class_id', $request->class)->where('section_id', $request->section)->where('id','!=', $request->id)->first();
        if ($is_duplicate) {
