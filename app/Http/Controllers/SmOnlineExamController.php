...
class SmOnlineExamController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        try{
            ...
            if ($result) {
                // $data = SmStudent::where('class_id', $request->class)->where('section_id', $request->section)->where('school_id',Auth::user()->school_id)->get();
                // if (!empty($data)) {
  ...
  public function delete(Request $request)
    ...
    public function manageOnlineExamQuestion($id)
    {
        ...
        try{
            ...
            $question_banks = SmQuestionBank::where('class_id', $online_exam->class_id)->where('section_id', $online_exam->section_id)->where('school_id',Auth::user()->school_id)->get();

            $assigned_questions = SmOnlineExamQuestionAssign::where('online_exam_id', $id)->where('school_id',Auth::user()->school_id)->get();
            $already_assigned = [];
    ...
    public function onlineExamPublish($id)
    {
        ...
        date_default_timezone_set($time_zone_setup->time_zone);
            ...
            $students = SmStudent::where('class_id', $class_id)->where('section_id', $section_id)->where('school_id',Auth::user()->school_id)->get();
    ...
    public function onlineExamMarksRegister($id)
    {
        ...
        try{
            ...
            $students = SmStudent::where('class_id', $online_exam_question->class_id)->where('section_id', $online_exam_question->section_id)->where('school_id',Auth::user()->school_id)->get();
            $present_students = [];
            foreach ($students as $student) {
    ...
    public function onlineExamResult($id)
    {
        try{
            ...
            $online_exam_question = SmOnlineExam::where('id',$id)->first();

            $students = SmCourseRegistration::where('sm_course_registrations.subject_id','=', $online_exam_question->subject_id)->where('sm_course_registrations.uploaded','=',0)
                                            ->join('sm_students','sm_course_registrations.student_id','sm_students.id')
                                            ->select('sm_students.*','sm_course_registrations.subject_id','sm_course_registrations.uploaded')
                                            ->get();
    ...
    public function onlineExamReport(Request $request)
    {
        try {
            if (moduleStatusCheck('OnlineExam')== TRUE) {
                $exams = InfixOnlineExam::where('active_status', 1)->where('school_id', Auth::user()->school_id)->get();
           
            } else {
                $exams = SmOnlineExam::where('active_status', 1)->where('school_id', Auth::user()->school_id)->get();
