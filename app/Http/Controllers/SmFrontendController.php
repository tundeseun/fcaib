...
//                      'is_registration_permission' => SmRegistrationSetting::where('registration_permission',1)->first(),
    ...
    public function studentRegistration($utme_number){

        if(DB::table('sm_fees_payments')->where('utme_number', $utme_number)->where('fees_type_id', 6)->exists()){
            Toastr::error('Complete Registration', 'Success');
            return redirect('/parentregistration/registration/'.$utme_number); 
        }
        ...
        try {
            ...
            $application_cost = getCost(6);

            $applicant_details = (object)[
                'utme_number' => Session::get('utme_number'),
                'email'       => Session::get('email'),
            ];

            $schools = SmSchool::where('active_status', 1)->get();
            return view('auth.studentRegistration', compact('schools', 'css', 'application_cost','utme_number', 'applicant_details'));
        } catch (\Exception $e) {
            Toastr::error('Operation Failed', 'Failed');
            return redirect()->back();
        }
    ...
    public function postutme(){
        ...
        if(DB::table('sm_fees_payments')->where('utme_number', Session::get('utme_number'))->where('fees_type_id', 6)->exists()){
            Toastr::error('Complete Registration', 'Success');
            return redirect('/parentregistration/registration/'.Session::get('utme_number')); 
        }
        ...
        try {
            ...
            $schools = SmSchool::where('active_status', 1)->get();
            return view('auth.postutme', compact('schools', 'css',));
        } catch (\Exception $e) {
            Toastr::error('Operation Failed', 'Failed');
            return redirect()->back();
        } 
    ...
    public function postUtmeHandler(Request $request){
        try {
            $request->validate([
            ...
            if(DB::table('sm_fees_payments')->where('utme_number', $request->utme_number)->where('fees_type_id', 6)->exists()){
                Toastr::error('Complete Registration', 'Success');
                return redirect('/parentregistration/registration/'.$request->utme_number);              
            }else{
                Session::put('utme_number', $request->utme_number);
                Session::put('email', $request->email);
                Toastr::error('Make Payment', 'Success');
                return redirect('/student-registration/'.$request->utme_number);
            }
    ...
    public function postUtmeTwo(Request $request)
    {
        ...
        if($data->message == "SUCCESS" && $data->amount == $amount){
            ...
            $utme_number = $data->customerId;

            $user = (object)[
                "student" => (object)[ 
                    "matric_number" => $utme_number,
                    "full_name" => $data->firstName . ' '. $data->lastName
                ],
            ];

            //todo show hostel receipt
            $file =  'public/uploads/student/document/Post-UTME-Payment-' . md5(time()). '.' . 'pdf';
            $pdf = \App::make('dompdf.wrapper');
            $pdf->loadHTML(view("backEnd.studentPanel.student_receipt", compact('user', 'payment'))->render());
            $pdf->setPaper('A4', 'portrait');
            $pdf->save($file);

            $this->collectFee($amount, 'Accomodation', 4, $user, $transactionId, $file, 'Remita');

            $income_head = generalSetting();
            $fees_payment = new SmFeesPayment();
            $fees_payment->fees_type_id = 6;
            $fees_payment->payment_type = 'Post UTME';
            ...
            $fees_payment->payment_mode = 'Remita';
            $fees_payment->school_id = 1;
            $fees_payment->transaction_id = $transactionId;
            $fees_payment->receipt = $file;
            $fees_payment->email = $data->email;
            $fees_payment->utme_number = $utme_number;
            $fees_payment->save();

            $add_income = new SmAddIncome();
            $add_income->name = 'Post UTME';
            ...
            Mail::to($data->email)->later(now()->addSeconds(3), new SendUTMEMail($user, $file));

            Toastr::success('Payment successful, receipt has been sent to your email', 'Success');
            return redirect('/parentregistration/registration/'.$utme_number); 
