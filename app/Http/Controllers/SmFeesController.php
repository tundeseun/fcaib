...
class SmFeesController extends Controller
{
    ...
    public function collectFeesSearch(Request $request)
    {
        ...
        try {
            ...
            $students = $students->where('school_id',Auth::user()->school_id)->where('active_status',1)->get();
    ...
    {
        try{
            $fees_payments = SmFeesPayment::with(['studentInfo','feesType'])->get();
    ...
    public function searchFeesDue(Request $request)
    {
        try {
            ...
            $fees_dues = [];
            foreach ($students as $student) {
                   $fees_master = SmFeesMaster::select('id', 'amount','date')->where('school_id',Auth::user()->school_id)->first();
                ...
                $discount_amount = SmFeesAssign::where('student_id', $student->id)->where('fees_master_id', @$fees_master->id)->sum('applied_discount');
                $amount = SmFeesPayment::where('active_status',1)->where('student_id', $student->id)->sum('amount');
    ...
    public function feesDueSearch(Request $request)
    {
        ...
        try {
            ...
            $fees_dues = [];
            foreach ($students as $student) {
                   $fees_master = SmFeesMaster::select('id', 'amount','date')->where('fees_group_id', $fees_group[0])->where('fees_type_id', $fees_group[1])->where('school_id',Auth::user()->school_id)->first();
                    $total_amount = $fees_master->amount;
                ...
                $discount_amount = SmFeesAssign::where('student_id', $student->id)->where('fees_master_id', $fees_master->id)->sum('applied_discount');
                $amount = SmFeesPayment::where('active_status',1)->where('student_id', $student->id)->where('fees_type_id', $fees_group[1])->sum('amount');
    ...
    public function bankPaymentSlipSearch(Request $request)
    {
        ...
        try {
            ...
            $all_bank_slips = $bank_slips->where('school_id',Auth::user()->school_id)->orderBy('id', 'desc')->get();
    ...
    public function approveFeesPayment(Request $request){
        try {
          ...
          if (checkAdmin()) {
            ...
            $bank_slips = $bank_slips->where('school_id',Auth::user()->school_id)->orderBy('id', 'desc')->get();
            $date = $request->payment_date;
            $class_id = $request->class;
            $section_id = $request->section;
            $classes = SmClass::where('active_status', 1)->where('school_id',Auth::user()->school_id)->get();
