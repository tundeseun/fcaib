...
class SmClassTimeController extends Controller
{
    ...
    {
        try {
            $class_times = SmClassTime::where('type','class')->latest()->where('school_id', Auth::user()->school_id)->get();
    ...
    public function examTime(Request $request){
        try {
            $class_times = SmClassTime::where('type','exam')->latest()->where('school_id', Auth::user()->school_id)->get();
            if (ApiBaseMethod::checkUrl($request->fullUrl())) {
                return ApiBaseMethod::sendResponse($class_times, null);
            }
