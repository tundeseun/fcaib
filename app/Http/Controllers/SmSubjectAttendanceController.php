...
class SmSubjectAttendanceController extends Controller
{
    ...
    public function search(Request $request)
    {
        ...
        try{
            ...
            $classes = SmClass::where('active_status', 1)->where('school_id',Auth::user()->school_id)->get();
            $sections = SmClassSection::with('sectionName')->where('active_status', 1)
                ->where('school_id',Auth::user()->school_id)->where('class_id', $class)->get();
            $subjects = SmAssignSubject::with('subject')->where('active_status', 1)
                ->where('school_id',Auth::user()->school_id)->where('class_id', $class)->where('section_id', $section_id)
                ->groupBy('subject_id')->get();
            $students = SmStudent::where('class_id', $class)->where('section_id', $section_id)->where('active_status', 1)
            ...
            foreach ($students as $student) {
                $attendance = SmSubjectAttendance::where('student_id', $student->id)->where('subject_id', $subject_id)->where('attendance_date', date('Y-m-d', strtotime($request->attendance_date)))->first();
