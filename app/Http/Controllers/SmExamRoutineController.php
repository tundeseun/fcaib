...
class SmExamRoutineController extends Controller
{
    ...
    public function examScheduleCreate($class_id = 0, $section_id = 0)
    {
            ...
            if($class_id != 0 && $section_id != 0){
                ...
                $subjects =  SmSubject::where('class_id', $class_id)
                                            ->orWhere('class_id',0)
                                            ->orWhere('class_id',-1)
                                            ->where('section_id', $section_id)
                                            ->where('school_id',Auth::user()->school_id)
                                            ->get();
                $exam_dates = SmExamSchedule::where('class_id',$class_id)->where('section_id',$section_id)->groupby('day')->get();
            }else{
                $rooms = "";
                $subjects = "";
                $exam_dates = "";
            ...
            return view('backEnd.examination.exam_schedule_create', compact('classes', 'class_id', 'section_id', 'rooms','subjects','exam_dates'));

    }

    public function editExamSchedule($id,$section_id,$class_id)
    {
        ...
        try {
            ...
            $schedule = SmExamSchedule::with(['subject','classRoom'])->where('id',$id)->first();

            return view('backEnd.academics.edit_exam_schedule_form', compact('rooms', 'subjects','class_id', 'section_id','schedule'));
        } catch (\Exception $e) {
            Toastr::error('Operation Failed', 'Failed');
            return redirect()->back();
        }
    ...
    public function updateExamSchedule(Request $request){
        try {
            ...
            $data['date'] = date('Y-m-d', strtotime($request->day));
            $data['day'] = date('Y-m-d', strtotime($request->day));
            ...
            SmExamSchedule::where('id', $request->id)->update($data);
    ...
    public function addNewExamSchedule(Request $request){
        try {
        ...
        public function deleteExamScheduleModal($id)
    {

        try {
            return view('backEnd.academics.delete_exam_schedule', compact('id'));
        } catch (\Exception $e) {
            Toastr::error('Operation Failed', 'Failed');
            return redirect()->back();
        }
    ...
    public function deleteExamSchedule($id)
    {
        ...
        try {
            SmExamSchedule::find($id)->delete();
            Toastr::success('Exam Schedule has been deleted successfully', 'Success');
    ...
    public function examRoutineView($class_id, $section_id, $exam_term_id)
    {
        ...
        try {
            if($section_id==0){
                $assign_subjects = SmAssignSubject::where('class_id', $class_id)->groupby(['section_id','subject_id'])->where('school_id',Auth::user()->school_id)->get();

            }else{
                $assign_subjects = SmAssignSubject::where('class_id', $class_id)->groupby(['section_id','subject_id'])->where('section_id', $section_id)->where('school_id',Auth::user()->school_id)->get();
    ...
    public function checkExamRoutineDate(Request $request)
    {
        ...
        try {
            if ($request->assigned_id == "") {
                $check_date = SmExamSchedule::where('class_id', $request->class_id)->where('section_id', $request->section_id)->where('exam_term_id', $request->exam_term_id)->where('date', date('Y-m-d', strtotime($request->date)))->where('exam_period_id', $request->exam_period_id)->where('school_id',Auth::user()->school_id)->get();
            } else {
                $check_date = SmExamSchedule::where('id', '!=', $request->assigned_id)->where('class_id', $request->class_id)->where('section_id', $request->section_id)->where('exam_term_id', $request->exam_term_id)->where('date', date('Y-m-d', strtotime($request->date)))->where('exam_period_id', $request->exam_period_id)->where('school_id',Auth::user()->school_id)->get();
            }
    ...
    public function examRoutineReportSearch(Request $request)
    {
        ...
        try {
            $exam_types = SmExamType::where('academic_id', getAcademicId())->where('school_id',Auth::user()->school_id)->get();
            $exam_periods = SmClassTime::where('type', 'exam')->where('school_id',Auth::user()->school_id)->get();
            $exam_routines = SmExamSchedule::where('exam_term_id', $request->exam)->orderBy('date', 'ASC')->where('school_id',Auth::user()->school_id)->get();
    ...
    public function examRoutineReportSearchPrint($exam_id)
    {
        ...
        try {
            $exam_types = SmExamType::where('academic_id', getAcademicId())->where('school_id',Auth::user()->school_id)->get();
            $exam_periods = SmClassTime::where('type', 'exam')->where('school_id',Auth::user()->school_id)->get();
            $exam_routines = SmExamSchedule::where('exam_term_id', $exam_id)->orderBy('date', 'ASC')->where('school_id',Auth::user()->school_id)->get();
            $exam_routines = $exam_routines->groupBy('date');
            $academic_year = SmAcademicYear::find(getAcademicId());
