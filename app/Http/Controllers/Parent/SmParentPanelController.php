...
class SmParentPanelController extends Controller
{
    ...
    {
        try {
            $holidays = SmHoliday::where('active_status', 1)->where('school_id', Auth::user()->school_id)->get();
    ...
    public function myChildren($id)
    {
        ...
        try {
            ...
            if ($student_detail) {
                ...
                $leave_details = SmLeaveRequest::where('staff_id', $student_detail->user_id)->where('role_id', 2)->where('active_status', 1)->where('school_id', Auth::user()->school_id)->get();
    ...
    function leaveApply(Request $request)
    {
        try {
            ...
            $std_id = SmStudent::leftjoin('sm_parents','sm_parents.id','sm_students.parent_id')
                ...
                $apply_leaves = SmLeaveRequest::where('staff_id', $std_id->user_id)->where('role_id', 2)->where('active_status', 1)->where('school_id', Auth::user()->school_id)->get();
                $leave_types = SmLeaveDefine::where('role_id', 2)->where('active_status', 1)->where('school_id', Auth::user()->school_id)->get();
...
//                'attach_file' => "sometimes|nullable|mimes:pdf,doc,docx,jpg,jpeg,png,txt",
    ...
    {
        try {
            $apply_leaves = SmLeaveRequest::where([['active_status', 1], ['approve_status', 'P']])
                ->where('school_id', Auth::user()->school_id)->get();
            $leave_types = SmLeaveType::where('active_status', 1)->whereOr(['school_id', Auth::user()->school_id],
                ['school_id', 1])->get();
    ...
    public function parentLeaveEdit(request $request, $id)
    {
        try {
            ...
            if ($user) {
                $my_leaves = SmLeaveDefine::where('role_id', 2)->where('school_id', Auth::user()->school_id)->get();
                $apply_leaves = SmLeaveRequest::where('role_id', 2)->where('active_status', 1)->where('school_id', Auth::user()->school_id)->get();
                $leave_types = SmLeaveDefine::where('role_id', 2)->where('active_status', 1)->where('school_id', Auth::user()->school_id)->get();
            } else {
