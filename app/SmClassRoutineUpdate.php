...
class SmClassRoutineUpdate extends Model
{
    public static function assingedClassRoutine($class_time, $day, $class_id, $section_id)
    {
        try {
            ...
            return SmClassRoutineUpdate::where('class_period_id', $class_time)->where('day', $day)->where('class_id', $class_id)->where('section_id', $section_id)->first();
        } catch (\Exception $e) {
            $data=[];
            return $data;
        }
    ...
    {
        try {
            return SmClassRoutineUpdate::where('class_period_id', $class_time)->where('day', $day)->where('class_period_id', $class_time)->where('school_id',Auth::user()->school_id)->where('teacher_id', $teacher_id)->first();
        } catch (\Exception $e) {
            $data=[];
            return $data;
        }
