...
class SmStudent extends Model
{
    ...
    {
        return $this->hasMany(SmStudentAttendance::class, 'student_id')->where('school_id', Auth::user()->school_id);
    }
    ...
    {
        $eligible_subjects = SmAssignSubject::where('class_id', $student->class_id)->where('section_id', $student->section_id)->groupby(['section_id', 'subject_id'])
    ...
    {
        return $this->hasOne(SmOptionalSubjectAssign::class, 'student_id')->where('session_id', $this->session_id);
    }
    ...
    {
        return $this->hasOne(SmGraduandsList::class, 'student_id');
    }

    public function owes()
    {
        //dd($this->feesAssign);
        return $this->feesAssign->count() > $this->feesPayments()->onlyAssigned()->count();
    }
}
