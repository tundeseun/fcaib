...
use App\Http\Controllers\SmFeesController;
use App\Http\Controllers\RemitaPaymentController;
use App\Http\Controllers\RemitaPaymentController2;
use App\Http\Controllers\ResponseController;

Route::get('/check-rrr-status', [RemitaPaymentController2::class, 'showForm'])->name('check-rrr-status-form');
Route::post('/check-rrr-status', [RemitaPaymentController2::class, 'checkrrrstatus'])->name('check-rrr-status');

// Route::get('/payment-success/redirect', function() {
//      return view('payment.success', compact('payment'));// Adjust the view path as necessary
// });
// Route::get('/payment-success/{payment_id}', [ResponseController::class, 'paymentSuccess'])->name('payment.success');

// Route::post('/save-response/{user_id}', [ResponseController::class, 'store'])->name('save-response');


Route::post('/save-response/{user_id}/{fees_type_id}', [ResponseController::class, 'store'])->name('save-response');
Route::get('/payment-success/{payment_id}', [ResponseController::class, 'paymentSuccess'])->name('payment.success');




Route::get('/payment-form', [RemitaPaymentController2::class, 'showPaymentForm'])->name('payment.form');
Route::post('/initiate-payment', [RemitaPaymentController2::class, 'initiatePayment'])->name('initiate.payment');


Route::get('/initiate-payment', [RemitaPaymentController2::class, 'showPaymentForm'])->name('initiate.payment');
Route::post('/initiate-payment', [RemitaPaymentController2::class, 'initiatePayment'])->name('initiate.payment.post');



Route::post('/generate-rrr', [RemitaPaymentController::class, 'generateRrr'])->name('generate-rrr');

Route::get('/payment-details', [RemitaPaymentController::class, 'paymentDetails'])->name('payment-details');
...
/*    Route::get('home',  'SmFrontendController@index');
    ...
    Route::get('student-registration/{utme_number}', 'SmFrontendController@studentRegistration')->name('student-registration');

    Route::get('post-utme', 'SmFrontendController@postutme')->name('post-utme');
    Route::post('post-utme-handler', 'SmFrontendController@postUtmeHandler')->name('post-utme-handler');
    Route::get('post-utme-2/{paymentReference}/{transactionId}', 'SmFrontendController@postUtmeTwo')->name('post-utme-2');
    ...
    Route::post('news-heading-update', 'SmFrontendController@newsHeadingUpdate')->middleware('userRolePermission:524');
    ...
    Route::post('course-heading-update', 'SmFrontendController@courseHeadingUpdate')->middleware('userRolePermission:526');
